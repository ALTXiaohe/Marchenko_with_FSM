                ...
                int nw, int nw_low, int ngath, int nx, int nxm, int ntfft, int mode, float weight, float tsq, int verbose)
{
    ...
    ctrace = (float complex *)malloc(ntfft*sizeof(float complex));
    ...
    while (!end_of_file) {
        ...
        while (one_shot) {
            ...
            DFFT(ctrace, trace, ntfft, ntfft);

            for (iw=0; iw<nw; iw++) {
                cdata[igath*nx*nw+iw*nx+itrace] =weight*crealf(ctrace[nw_low+iw]) + (weight*mode*cimgf(ctrace[nw_low+iw]) * I;
            }
            itrace++;
            xnx[igath]+=1;

...
int readShotData(char *filename, float *xrcv, float *xsrc, float *zsrc, int *xnx, float complex *cdata,
                int nw, int nw_low, int ngath, int nx, int nxm, int ntfft, int mode, float weight, float tsq, int verbose);
int readTinvData(char *filename, float *xrcv, float *xsrc, float *zsrc, int *xnx, int Nsyn, int nx,
                int ntfft, int mode, int *maxval, float *tinv, int hw, int verbose);
int writeDataIter(char *file_iter, float *data, segy *hdrs, int n1, int n2, float d2, float f2, int n2out, 
                int Nsyn, float *xsyn, float *zsyn, int iter);
...
int getFileInfo(char *filename, int *n1, int *n2, int *ngath, float *d1, float *d2, float *f1, float *f2, 
                float *xmin, float *xmax, float *sclsxgx, int *nxm);
int readData(FILE *fp, float *data, segy *hdrs, int n1);
int writeData(FILE *fp, float *data, segy *hdrs, int n1, int n2);
int disp_fileinfo(char *file, int n1, int n2, float f1, float f2, float d1, float d2, segy *hdrs);
double wallclock_time(void);

void synthesis(float complex *Refl, float complex *Fop, float *Top, float *iRN, int nx, int nt, 
                int nxs, int nts, float dt, float *xsyn, int Nsyn, float *xrcv, float *xsrc, 
                float fxs2, float fxs, float dxs, float dxsrc, float dx, int ixa, int ixb, 
                int ntfft, int nw, int nw_low, int nw_high,  int mode, int reci, int nshots, 
                int *ixpossyn, int npossyn, double *tfft, int verbose);

void synthesisPosistions(int nx, int nt, int nxs, int nts, float dt, float *xsyn, int Nsyn, 
                float *xrcv, float *xsrc, float fxs2, float fxs, float dxs, float dxsrc, 
                float dx, int ixa, int ixb,  int reci, int nshots, int *ixpossyn, int *npossyn, int verbose);

/*********************** self documentation **********************/
char *sdoc[] =
{
    " ",
    " MARCHENKO - Iterative Green's function and focusing functions retrieval with free-surface multiples",
    " ",
...
int main (int argc, char **argv)
{
    ...
    nshots = ngath;
    assert (nxs >= nshots); //If this condition is true, continue, or end.
    ...
    Fop     = (float complex *)calloc(nxs * nw * Nsyn, sizeof(float complex));
    ...
    rNi      = (float *)calloc(Nsyn * nxs * ntfft, sizeof(float));
    riRN     = (float *)calloc(Nsyn * nxs * ntfft, sizeof(float));
    ...
    Refl    = (float complex *)malloc(nw * nx * nshots * sizeof(float complex));
    ...
    rmode = 1; /* apply normal complex to read in data */
	...
	/* Similar to the reducing of Gibbs effection */ 
    if (tap == 1 || tap == 3)
    {
        for (j = 0; j < ntap; j++)
    ...
    memcpy(rNi, G_d, Nsyn * nxs * ntfft * sizeof(float));
    ...
    for (iter = 0; iter < niter; iter++)
    {
        ...
        synthesis(Refl, Fop, rNi, riRN, nx, nt, nxs, nts, dt, xsyn, Nsyn,
                  xrcv, xsrc, fxs2, fxs, dxs, dxsrc, dx, ixa, ixb, ntfft, nw, nw_low, nw_high, rmode,
                  reci, nshots, ixpossyn, npossyn, &tfft, verbose);
...
void synthesis(float complex *Refl, float complex *Fop, float *Top, 
                float *iRN, int nx, int nt, int nxs, int nts, float dt, 
                float *xsyn, int Nsyn, float *xrcv, float *xsrc, float fxs2, 
                float fxs, float dxs, float dxsrc, float dx, int ixa, int ixb, 
                int ntfft, int nw, int nw_low, int nw_high,  int mode, int reci, 
                int nshots, int *ixpossyn, int npossyn, double *tfft, int verbose)
{
    int     nfreq, size, iox, inx;
    float   scl;
    int     i, j, l, m, iw, ix, k;
    float   *rtrace, idxs;
    float complex *sum, *ctrace;
    int     npe;
    ...
    ctrace = (float complex *)calloc(ntfft, sizeof(float complex));
    if (!first)
    {
        ...
        for (l = 0; l < Nsyn; l++)
        {
			...
			memset(&Fop[l * nxs * nw], 0, nxs * nw * 2 * sizeof(float));
            for (i = 0; i < npossyn; i++)
            {
				DFFT(ctrace, &Top[l * size + i * nts], nts, ntfft);
                ix = ixpossyn[i];
                for (iw = 0; iw < nw; iw++)
                {
					Fop[l * nxs * nw + iw * nxs + ix] =crealf(ctrace[nw_low + iw]) + (mode * cimag(ctrace[nw_low + iw])) * I;						
                }
            }
        }
    }
    else   /* only for first call to synthesis */
    {
        ...
        for (l = 0; l < Nsyn; l++)
        {
            ...
            for (i = 0; i < nxs; i++)
            {
                ...
                for (iw = 0; iw < nw; iw++)
                {
					Fop[l * nxs * nw + iw * nxs + i] = crealf(ctrace[nw_low + iw]) + (mode * cimag(ctrace[nw_low + iw])) * I;		
                }
            }
        }
        idxs = 1.0 / dxs;
        ...
        {
            for (i = 0; i < nx; i++)
            {	/////Regulate the first reveiver's position to zero ///// 
    ...
    *tfft += t1 - t0;
    ...
    for (k = 0; k < nshots; k++)
    {
        ...
        private(l, ix, j, m, i, sum, rtrace)
        {
            ...
            sum   = (float complex *)malloc(nfreq * sizeof(float complex));
            ...
            for (l = 0; l < Nsyn; l++)
            {
                ...
                memset(&sum[0], 0, nfreq * 2 * sizeof(float));

                for (j = nw_low, m = 0; j <= nw_high; j++, m++)
                {
                    for (i = iox; i < inx; i++)
                    {
						sum[j] += Refl[k * nw * nx + m * nx + i] * Fop[l * nw * nxs + m * nxs + ixrcv[k * nx + i]];
                    }
                }

                /* transfrom result back to time domain */
                IDFFT(sum, rtrace, nts, ntfft);
...
void synthesisPosistions(int nx, int nt, int nxs, int nts, float dt, float *xsyn, 
                        int Nsyn, float *xrcv, float *xsrc, float fxs2, float fxs, 
                        float dxs, float dxsrc, float dx, int ixa, int ixb,  int reci, 
                        int nshots, int *ixpossyn, int *npossyn, int verbose)
{
    int     iox, inx;
